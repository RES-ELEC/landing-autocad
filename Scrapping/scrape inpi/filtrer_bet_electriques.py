# -*- coding: utf-8 -*-
"""
Filtrage BET OPQIBI Electricite - v4 (selection fichier via boite de dialogue)
...
Usage       : python filtrer_bet_electriques.py
              (une boite de dialogue s'ouvre pour choisir le ou les fichiers Excel)
"""
import os
import re
import sys
...
def choisir_fichiers():
    """Ouvre une boite de dialogue pour selectionner un ou plusieurs fichiers Excel."""
    root = tk.Tk()
    root.withdraw()
    root.attributes("-topmost", True)
    fichiers = filedialog.askopenfilenames(
        title="Selectionnez le ou les fichiers Excel a analyser",
...
def choisir_fichier_sortie(defaut):
    """Ouvre une boite de dialogue pour choisir ou sauvegarder le fichier de sortie."""
    root = tk.Tk()
    root.withdraw()
    root.attributes("-topmost", True)
    fichier = filedialog.asksaveasfilename(
        title="Enregistrer le fichier resultat sous...",
        defaultextension=".xlsx",
        initialfile=os.path.basename(defaut),
        initialdir=os.path.dirname(defaut),
...
def charger_lignes(fichiers):
    """Charge toutes les lignes (a partir de la ligne 2) depuis les fichiers selectionnes."""
    rows = []
    for fichier in fichiers:
        try:
            ...
            for r in ws.iter_rows(min_row=2, values_only=True):
                if r and r[0]:
                    rows.append(r)
            print(f"  -> {os.path.basename(fichier)} : {len(rows) - n_avant} lignes")
...
def main():
    print("[*] Selection du ou des fichiers Excel a analyser...")
    fichiers = choisir_fichiers()
    if not fichiers:
        print("[!] Aucun fichier selectionne. Abandon.")
        sys.exit(0)

    print(f"[*] {len(fichiers)} fichier(s) selectionne(s) :")
    for f in fichiers:
        print(f"    - {f}")

    rows = charger_lignes(fichiers)
    if not rows:
        print("[!] Aucune ligne exploitable trouvee.")
        sys.exit(1)

    # Fichier de sortie : propose a cote du premier fichier d'entree
    premier = fichiers[0]
    defaut_sortie = os.path.join(
        os.path.dirname(premier),
        os.path.splitext(os.path.basename(premier))[0] + "_CIBLES.xlsx"
    )
    print("[*] Selection du fichier de sortie...")
    fichier_out = choisir_fichier_sortie(defaut_sortie)
    if not fichier_out:
        print("[!] Aucun fichier de sortie selectionne. Abandon.")
        sys.exit(0)
    ...
    with cf.ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        ...
        for i, f in enumerate(cf.as_completed(futs), 1):
            ...
            try:
                ...
                res = {"nom": r[0], "url": r[3] if len(r) > 3 else "", "url_final": "",
                       "score": 0, "hits": [], "detail": "",
                       "preuve": "", "erreur": f"crash: {e}", "pages_lues": 0}
            res["gerant"] = r[1] if len(r) > 1 and r[1] else ""
            res["email"]  = r[2] if len(r) > 2 and r[2] else ""
            results.append(res)
    ...
    try:
        ...
        try:
            ...
            messagebox.showinfo(
                "Termine",
                f"Analyse terminee !\n\n"
                f"{len(cibles)} cibles retenues / {len(results)} societes\n\n"
                f"Fichier :\n{fichier_out}"
            )
            root.destroy()
        except Exception:
        ...
        print(f"[ERREUR] Impossible d'ecrire {fichier_out} (fichier ouvert ?)")
        sys.exit(1)
